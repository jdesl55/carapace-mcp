...
export class LogStore {
   ...
   */
  async saveReview(review: ReviewEntry): Promise<LogResult> {
    if (!this.db) {
      throw new Error("LogStore not initialized. Call initialize() first.");
    }
    ...
    const result = stmt.run(
      ...
      review.goal_alignment_score,
      review.security_compliance_score,
      review.constraint_adherence_score,
      review.total_actions,
      review.verified_actions,
      review.blocked_actions,
  ...
  getReviewsByDateRange(startDate: string, endDate: string): any[] {
    ...
    const rows = this.db
      .prepare(
        "SELECT * FROM reviews WHERE timestamp >= ? AND timestamp <= ? ORDER BY timestamp DESC"
      )
