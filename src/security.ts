...
export class SecurityEngine {
  ...
  private rotationMinutes: number = 30;
  private dailySpendUsed: number = 0;
  private dailySpendResetDate: string = "";
  ...
  checkRules(action: ActionCheck): RuleResult {
    ...
    if (["send_message", "send_email"].includes(action.action_type)) {
      ...
      const blockedContacts: string[] = this.config.get("security.contacts.blocked", []);
      const allowedContacts: string[] = this.config.get("security.contacts.allowed", []);
      const contactMode = this.config.get("security.contacts.mode", "blocklist" as string);
      ...
      if (contactMode === "allowlist" && allowedContacts.length > 0) {
        const isAllowed = allowedContacts.some(
          (c) => targetLower.includes(c.toLowerCase())
        );
        if (!isAllowed) {
          allowed = false;
    ...
    if (["browse_new_domain", "api_write"].includes(action.action_type)) {
      ...
      const blockedDomains: string[] = this.config.get("security.domains.blocked", []);
      const allowedDomains: string[] = this.config.get("security.domains.allowed", []);
      const domainMode = this.config.get("security.domains.mode", "blocklist" as string);

      const targetLower = action.target.toLowerCase();

      if (domainMode === "allowlist" && allowedDomains.length > 0) {
        const isAllowed = allowedDomains.some(
          (d) => targetLower.includes(d.toLowerCase())
        );
        if (!isAllowed) {
          allowed = false;
    ...
    const blockedActions: string[] = this.config.get("security.blockedActions", []);
    ...
    const customRules: Array<{
      if: { field: string; operator: string; value: string | number };
      then: "block" | "warn";
      reason: string;
    }> = this.config.get("security.customRules", []);

    if (customRules.length > 0) {
      rulesChecked.push("custom_rules");

      for (const rule of customRules) {
        const fieldValue = (action as any)[rule.if.field];
        let matches = false;
  ...
  private computeKey(windowIndex: number): string {
    const hmac = crypto.createHmac("sha256", this.secret);
    hmac.update(String(windowIndex));
    return hmac.digest("hex").substring(0, 16);
  }

  private getDailySpendRemaining(): number {
    this.resetDailySpendIfNeeded();
