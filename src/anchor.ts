...
interface AnchorBlock {
  goals: string[];
  priorities: Array<{ rank: number; text: string }>;
  constraints: string[];
  context: string;
  message: string;
}
...
interface DriftAssessment {
  ...
  score: number; // 0-1 where 0 = perfect alignment, 1 = total drift
  explanation: string;
...
export class AnchorEngine {
  ...
  private currentDriftLevel: string = "none";
  ...
  getAnchorBlock(): AnchorBlock {
    ...
    const goals: string[] = this.config.get("anchor.goals", []);
    const priorities: Array<{ rank: number; text: string }> = this.config.get(
      "anchor.priorities",
      []
    );
    const constraints: string[] = this.config.get("anchor.constraints", []);
    const context: string = this.config.get("anchor.context", "");
    ...
    return {
      ...
      constraints,
      context,
      message: this.composeHumanReadableAnchor(
        goals,
        sortedPriorities,
        constraints,
        context
  ...
  assessDrift(activitySummary: string): DriftAssessment {
    ...
    if (driftScore < 0.2) level = "none";
    else if (driftScore < 0.4) level = "low";
    else if (driftScore < 0.7) level = "medium";
    else level = "high";
  ...
  getStatus(): AnchorStatus {
    const now = new Date();
    const refreshInterval = this.config.get(
      "anchor.refreshIntervalMinutes",
      15
    );
