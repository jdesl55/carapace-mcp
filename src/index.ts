  ...
  "carapace_verify",
  {
    ...
    inputSchema: {
      ...
      target: z.string().describe(
        "Who/what the action targets. For messages: recipient name/address. " +
        "For purchases: merchant/item. For file ops: file path. For API: endpoint URL."
      ),
      amount: z.number().optional().describe(
        "Dollar amount if the action involves spending money"
      ),
      description: z.string().describe(
        "Brief human-readable description of what the agent intends to do"
      ),
      current_key: z.string().optional().describe(
        "The verification_key from the most recent carapace_verify call. " +
...
server.registerTool(
  "carapace_anchor",
  {
    ...
    description:
      "Call this at the start of every session and every 15 minutes during operation. " +
    ...
    inputSchema: {
      context_summary: z.string().optional().describe(
        "Brief summary of what you've been doing since the last anchor call. " +
          ...
          text: JSON.stringify(
            {
              ...
              timestamp: new Date().toISOString(),
              message:
                "This is your grounding context. Your user configured these goals and " +
...
server.registerTool(
  "carapace_log",
  {
    ...
    description:
      "Call this AFTER completing any significant action to record it in the " +
    ...
    inputSchema: {
      ...
      result: z.enum(["success", "failure", "partial"]).describe(
        "Outcome of the action"
      ),
      tier: z.enum(["1", "2", "3"]).describe(
        "Sensitivity tier: 1=highest (financial, messaging), 2=medium (browsing, installs), 3=low (reads, lookups)"
      ),
      amount: z.number().optional().describe(
        "Dollar amount if the action involved spending"
      ),
      verification_key: z.string().optional().describe(
        "The verification_key used for this action (if it was verified first)"
      ),
    },
    annotations: {
          ...
          text: JSON.stringify(
            {
              ...
              health:
                sessionStats.unverifiedTier1 > 0
                  ? "WARNING"
  ...
  "carapace_review",
  {
    ...
    inputSchema: {
      session_id: z.string().optional().describe(
        "Specific session ID to review. If omitted, reviews the current session."
      ),
      save: z.boolean().optional().describe(
        "Whether to save this review to history and update insights.md. Defaults to true."
      ),
    },
    annotations: {
      readOnlyHint: false,
  ...
  async ({ session_id, save }) => {
    ...
    if (shouldSave) {
      await logs.saveReview({
        ...
        highlights: sessionReview.highlights as any,
